        ...
        module Forker
            ...
            def fork_worker(worker)
                fork do
                    ...
                    logger.info "Starting #{worker.name} worker in the background ..." rescue nil
                    Process.setproctitle(worker.name)
                    Process.setsid
                    pid = curr_pid
                    logger.info "#{worker.name} worker successfully started in the background (pid=#{pid})." rescue nil
                    logger.info "Refreshing #{worker.pid_file_path} file with #{pid}." rescue nil
                    refresh_pid_file(worker.pid_file_path, pid)
                    logger.info "#{worker.pid_file_path} successfully created with #{pid}." rescue nil
                    worker.run
                end
            end
        end
